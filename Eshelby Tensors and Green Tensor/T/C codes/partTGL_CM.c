...
void Inverse(double **A, double **B)  // inverse of a 3x3 matrix
{
	double det;
	...
	B[1][1]=(-A[0][2]*A[2][0] + A[0][0]*A[2][2])/det;
...
void partTGL(size_t n, double i, double j, double k, double l, double *weight, double *theta, double *phi, double *C, double *x, double *result){
	...
	for(jj=0; jj<n; jj++)
	{
	    ...
	    // inverse Av
		Inverse(Av, ivAv);
		ft[jj]=xi[(int)(j-1)]*xi[(int)(l-1)]*ivAv[(int)(i-1)][(int)(k-1)]*sin(phi[jj])/rho;		
	}
